                ...
                for link in links:
                    ...
                    try:
                        ...
                        if add_to_cart_button and not add_to_cart_button.get_attribute('disabled'):
                            ...
                            # Save the cart page HTML and screenshot
                            with open('cart.html', 'w', encoding='utf-8') as f:
                                f.write(driver.page_source)
                            driver.save_screenshot('cart.png')

                            # Find and click the checkout button
                            checkout_button = driver.find_element(By.ID, 'CartDrawer-Checkout')
                            if checkout_button and not checkout_button.get_attribute('disabled'):
                                checkout_button.click()
                                time.sleep(5)
                                driver.save_screenshot('checkout.png')
                                print(f"Added product to cart and proceeded to checkout: {url[:-1] + link}")
                            else:
                                print(f"Checkout button not found for {url[:-1] + link}")
                        else:
                            print(f"Product is sold out: {url[:-1] + link}")
                    except Exception as e:
                        print(f"Error occurred while trying to add product to cart: {url[:-1] + link}. Error: {e}")
